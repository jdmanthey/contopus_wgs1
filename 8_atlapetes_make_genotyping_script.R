...
##############    START OPTIONS     #####################################
	...
	project_directory <- "/lustre/scratch/jmanthey/19_mexico/atlapetes"
	directory_name <- "11_genotype_atlapetes"
	reference_genome_location <- "/home/jmanthey/references/GCA_028018845.1_bMelGeo1.pri_genomic.fna"
	cluster <- "quanah"
	output_name <- "atlapetes"
	popmap <- "popmap_atlapetes.txt"
	individuals <- read.table(popmap, sep="\t")
	individuals[,1] <- as.character(individuals[,1])
	faidx <- read.table("melospiza.fai", stringsAsFactors=F)
