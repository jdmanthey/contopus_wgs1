...
##############    START OPTIONS     #####################################
	...
	project_directory <- "/lustre/scratch/jmanthey/19_mexico/cardellina"
	directory_name <- "11_genotype_cardellina"
	reference_genome_location <- "/home/jmanthey/references/GCA_001746935.2_mywa_2.1_genomic.fna"
	cluster <- "quanah"
	output_name <- "cardellina"
	popmap <- "popmap_cardellina.txt"
	individuals <- read.table(popmap, sep="\t")
	individuals[,1] <- as.character(individuals[,1])
	faidx <- read.table("setophaga.fai", stringsAsFactors=F)
