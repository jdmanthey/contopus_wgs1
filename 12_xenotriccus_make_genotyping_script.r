...
##############    START OPTIONS     #####################################
	...
	project_directory <- "/lustre/scratch/jmanthey/19_mexico/xenotriccus"
	directory_name <- "11_genotype_xenotriccus"
	reference_genome_location <- "/home/jmanthey/references/flycatcher_rearranged.fa"
	cluster <- "quanah"
	output_name <- "xenotriccus"
	popmap <- "popmap_xenotriccus.txt"
	individuals <- read.table(popmap, sep="\t")
	individuals[,1] <- as.character(individuals[,1])
	faidx <- read.table("flycatcher_rearranged.fa.fai", stringsAsFactors=F)
