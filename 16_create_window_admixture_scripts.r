	options(scipen=999)
	project_directory <- "/lustre/scratch/jmanthey/15_contopus/14_50kbp_admixture"
	directory_name <- "admix_fly_50kbp"
	cluster <- "quanah"
	max_number_jobs <- 400
	...
	header <- paste('\tgunzip -cd ', project_directory, '/CHR_30.recode.vcf.gz | grep "#" > ', project_directory, "/windows/${chrom_array}__${start_array}__${end_array}.recode.vcf", sep="")
	...
	write("", file=a.script, append=T)
	# reformat in plink
	admixture_commands <- paste0("\tplink --file ", project_directory, "/windows/${chrom_array}__${start_array}__${end_array} --recode12 --allow-extra-chr --out ", project_directory, "/windows/${chrom_array}__${start_array}__${end_array}__b")
	write(admixture_commands, file=a.script, append=T)
	write("", file=a.script, append=T)
	# run admixture
	admixture_commands <- paste0("\tfor K in 2; do admixture -C 0.000001 --cv ",project_directory, "/windows/${chrom_array}__${start_array}__${end_array}__b.ped $K; done")
	write(admixture_commands, file=a.script, append=T)
	write("", file=a.script, append=T)
